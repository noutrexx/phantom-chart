  ...
  return (
    <div className="h-full overflow-y-auto pb-28 bg-[var(--color-bg)]">
      <div className="relative px-5 pt-3">
        <div className="relative h-48 overflow-hidden rounded-[22px] bg-[var(--color-soft)] shadow-soft">
      ...
      <div className="mx-5 mt-3 rounded-[18px] border border-[var(--color-line)] bg-[var(--color-surface)] p-4 shadow-soft">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-[25px] font-extrabold tracking-tight leading-tight">{restaurant.name}</h1>
        ...
        <div className="flex items-center gap-2 mt-3 text-[13px]">
          ...
          <span className="text-[var(--color-ink-3)]">({restaurant.reviewsCount.toLocaleString("en-US")})</span>
      ...
      <div className="sticky top-0 z-10 bg-[var(--color-bg)]/95 backdrop-blur border-b border-[var(--color-line)] mt-3">
      ...
      {tab === "menu" ? (
                ...
                {popular.map((item) => (
                  <button key={item.id} onClick={() => setSelected(item)} className="shrink-0 w-[136px] text-left active:scale-[0.98] transition">
                    ...
                    <div className="mt-1 flex items-center justify-between gap-2">
                      ...
                      <span className="rounded-full bg-[var(--color-ink)] px-2 py-0.5 text-[10.5px] font-extrabold text-white">Add</span>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {restaurant.sections.map((sec) => (
              <div id={`section-${slug(sec.title)}`} key={sec.title} className="scroll-mt-24 pt-6">
                <div className="mb-1 flex items-end justify-between">
                  <h3 className="text-[18px] font-extrabold tracking-tight">{sec.title}</h3>
                  <span className="text-[11.5px] font-bold text-[var(--color-ink-3)] tabular-nums">{sec.items.length} items</span>
                </div>
                <div>
                  {sec.items.map((item) => (
                    <MenuRow
        ...
        <div className="px-5 pt-5 flex flex-col gap-3">
          <div className="rounded-xl bg-[var(--color-soft)] p-4">
            ...
            <p className="text-[13px] font-bold mt-1">{restaurant.reviewsCount.toLocaleString("en-US")} phantom reviews</p>
    ...
    >
      ...
      <div className="relative w-[96px] h-[96px] shrink-0">
        ...
        <button
          ...
          className={`absolute -bottom-2 right-1 min-w-12 h-8 rounded-full bg-white px-3 grid place-items-center shadow-card active:scale-90 transition ${popped ? "animate-pop" : ""}`}
        >
          {qty > 0 ? (
            <span className="text-[12.5px] font-extrabold tabular-nums">{qty} in cart</span>
          ) : (
            <span className="flex items-center gap-1 text-[12.5px] font-extrabold">
              <Plus size={14} className="text-[var(--color-ink)]" /> Add
            </span>
          )}
        </button>
      </div>
    ...
    <div className="fixed inset-0 z-40 bg-black/35 flex items-end" onClick={onClose}>
      <div className="w-full bg-[var(--color-bg)] rounded-t-3xl max-h-[88%] flex flex-col shadow-lift fade-up" onClick={(e) => e.stopPropagation()}>
        <div className="overflow-y-auto">
          ...
          <div className="relative h-44 mx-4 mt-2 overflow-hidden rounded-[22px]">
            <FoodImage src={foodImg(item.photo, `${item.id}-sheet`, 720, 460)} alt={item.name} className="absolute inset-0" gradient={restaurant.gradient} />
            <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/72 to-transparent" />
            <button onClick={onClose} aria-label="Close item" className="absolute top-3 right-3 w-10 h-10 rounded-full bg-white grid place-items-center shadow-card active:scale-90 transition">
          ...
          <div className="px-5 pt-4 pb-4">
            ...
            {groups.map((g) => (
              <OptionSection key={g.id} title={g.label} hint={g.type === "single" ? "Required" : "Optional"}>
                {g.choices.map((choice) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full min-h-12 flex items-center justify-between rounded-xl border px-3.5 py-3 text-left transition ${
                        active
                          ? g.type === "single"
                    ...
                    >
                      <span className="flex min-w-0 items-center gap-2.5">
                        ...
                        <span className="min-w-0 truncate text-[14px] font-bold">{choice.label}</span>
                      </span>
                      <span className="shrink-0 pl-3 text-[13px] font-semibold text-[var(--color-ink-2)] tabular-nums">{choice.delta > 0 ? `+$${choice.delta.toFixed(2)}` : "Included"}</span>
                    </button>
                  );
                })}
              </OptionSection>
